...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                seg_prediction, class_prediction = self.predict_logits_from_preprocessed_data(data).cpu()
                ...
                if ofile is not None:
                    ...
                    r.append(
                        export_pool.starmap_async(
                            ...
                            ((seg_prediction, properties, self.configuration_manager, self.plans_manager,
                              self.dataset_json, ofile, save_probabilities),)
                        )
                    )
                else:
                ...
                classification_prediction = classification_logits.argmax(dim=1).cpu().numpy()
                case_id = os.path.basename(ofile) if ofile is not None else "unknown_case"
                classification_results.append({
                    "Name": f"{case_id}.nii.gz",
                    "Subtype": int(classification_prediction[0])  # Assuming single prediction per case
                })
            ...
            # Save classification results as a CSV
        if classification_results:
            classification_output_path = os.path.join(os.path.dirname(ofile), "subtype_results.csv")
            import pandas as pd
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        if self.verbose:
            print('predicting')
        seg_logits, class_logits = self.predict_logits_from_preprocessed_data(dct['data']).cpu()

        if self.verbose:
            print('resampling to original shape')
        if output_file_truncated is not None:
            export_prediction_from_logits(seg_logits, dct['data_properties'], self.configuration_manager,
                                          self.plans_manager, self.dataset_json, output_file_truncated,
                                          save_or_return_probabilities)

            # Save classification results (e.g., write to CSV)
            classification_result = class_logits.argmax(dim=1).cpu().numpy()
            classification_output_path = f"{output_file_truncated}_classification.csv"
            with open(classification_output_path, 'w') as f:
                f.write(f"Name,Subtype\n")
                f.write(f"{output_file_truncated}, {int(classification_result[0])}\n")

        else:
            # Return both segmentation and classification outputs
            seg_result = convert_predicted_logits_to_segmentation_with_correct_shape(
                seg_logits, self.plans_manager, self.configuration_manager, self.label_manager,
                dct['data_properties'], save_or_return_probabilities
            )
            classification_result = class_logits.argmax(dim=1).cpu().numpy()
            return seg_result, classification_result

        if save_or_return_probabilities:
            return seg_result[0], seg_result[1], classification_result
        else:
            return seg_result, classification_result
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for params in self.list_of_parameters:
            ...
            # Perform sliding window prediction for both heads
            logits_segmentation, logits_classification = self.predict_sliding_window_return_logits(data)

            if prediction_segmentation is None:
                prediction_segmentation = logits_segmentation.to('cpu')
                prediction_classification = logits_classification.to('cpu')
            else:
                prediction_segmentation += logits_segmentation.to('cpu')
                prediction_classification += logits_classification.to('cpu')

        if len(self.list_of_parameters) > 1:
            prediction_segmentation /= len(self.list_of_parameters)
            prediction_classification /= len(self.list_of_parameters)

        if self.verbose: print('Prediction done')
        torch.set_num_threads(n_threads)
        return prediction_segmentation, prediction_classification
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        seg_logits, class_logits = self.network(x)
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                ...
                seg_logits_mirror, class_logits_mirror = self.network(flipped_x)
                seg_logits += torch.flip(seg_logits_mirror, axes)
                class_logits += class_logits_mirror

            # Average over original and mirrored predictions
            seg_logits /= (len(axes_combinations) + 1)
            class_logits /= (len(axes_combinations) + 1)
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        predicted_class_logits = torch.zeros(
            (self.network.classification_head[-1].out_features,),
            dtype=torch.half,
            device=self.device
        )
        n_predictions = prediction = gaussian = workon = None
        ...
        try:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                seg_logits_patch, class_logits_patch = self._internal_maybe_mirror_and_predict(workon)[0].to(results_device)
                seg_logits_patch = seg_logits_patch.to(results_device)

                if self.use_gaussian:
                    seg_logits_patch *= gaussian

                predicted_seg_logits[sl] += seg_logits_patch
                n_predictions[sl[1:]] += gaussian

            predicted_seg_logits /= n_predictions

            predicted_class_logits /= len(slicers)
            # check for infs
            ...
            del predicted_seg_logits, n_predictions, predicted_class_logits, gaussian, workon
            empty_cache(self.device)
            empty_cache(results_device)
            raise e

        return predicted_seg_logits, predicted_class_logits
        ...
        with torch.no_grad():
            ...
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    ...
                    try:
                        predicted_seg_logits, predicted_class_logits = self._internal_predict_sliding_window_return_logits(data, slicers,
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            # Predict segmentation and classification logits
            seg_logits, class_logits = self.predict_logits_from_preprocessed_data(torch.from_numpy(data))
            ...
            if of is not None:
                ...
                ret.append(seg.result)

            # Process classification output
            classification_prediction = class_logits.argmax(dim=1).cpu().numpy()
            case_id = os.path.basename(of) if of is not None else "unknown_case"
            classification_results.append({
                "Name": f"{case_id}.nii.gz",
                "Subtype": int(classification_prediction[0])
            })

        # Save classification results to CSV
        if classification_results:
            classification_output_path = os.path.join(output_folder, "subtype_results.csv")
            import pandas as pd
            classification_df = pd.DataFrame(classification_results)
            classification_df.to_csv(classification_output_path, index=False)
            print(f"Classification results saved to {classification_output_path}")
