...
class PlainConvUNetWithClassification(PlainConvUNet):
    def __init__(self,
                 ...
                 deep_supervision: bool = False,
                 nonlin_first: bool = False,
                 classification_head: dict = None):
        """
        Adds a classification head to the PlainConvUNet.
        """
        ...
        # Initialize the classification head if specified
        if classification_head:
            self.classification_head = nn.Sequential(
                    nn.AdaptiveAvgPool3d(1),  # Reduces spatial dimensions to 1x1x1
                    nn.Flatten(),  # Flattens to a 1D vector
                    nn.Linear(self.encoder.output_channels[-1], classification_head["n_classes"])  # Matches encoder output size
            )
        else:
            self.classification_head = None

    def forward(self, x):
        segmentation_output = super().forward(x)  # Encoder and segmentation head outputs
        ...
        if self.classification_head:
            ...
            classification_output = self.classification_head(encoder_output[-1])
