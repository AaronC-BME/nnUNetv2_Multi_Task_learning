...
class DefaultPreprocessor(object):
    ...
    def run_case(self, image_files: List[str], seg_file: Union[str, None], plans_manager: PlansManager,
        ...
        # Extract filename to match with dataset.json entries
        image_filename = os.path.basename(image_files[0])  # Extract only the filename

        # Find the corresponding classification label in dataset.json
        classification_label = None  # Default to None if not found
        for entry in dataset_json['training']:
            if os.path.basename(entry['image']) == image_filename:
                classification_label = entry['classification_label']
                break  # Stop searching once a match is found

        # Add classification label to properties
        data_properties['classification_label'] = classification_label if classification_label is not None else -1  # Use -1 if missing
